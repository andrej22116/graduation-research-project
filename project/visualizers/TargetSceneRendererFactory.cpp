#include "TargetSceneRendererFactory.hpp"
...
constexpr ullong str_hash_for_switch(const char* const str)
{
    return (str_is_correct(str) && (str_len(str) <= MAX_LEN)) ? str_hash(str, str_len(str)) : N_HASH;
}

constexpr ullong str_hash_for_switch(const std::string& str)
{
